...
{
    /**
     * Inizializzazione dati essenziali: admin, utente test, categorie base, province italiane.
     * Idempotente: i record vengono inseriti solo se non già presenti.
     */
    public function up(): void
    {
        // ── Admin ────────────────────────────────────────────────────────
        if (! DB::table('admins')->where('email', 'admin@qualeadfied.com')->exists()) {
        ...
        // ── User test + ClientProfile ────────────────────────────────────
        if (! DB::table('users')->where('email', 'test@qualeadfied.com')->exists()) {
        ...
        // Province non usa timestamps (modello senza timestamps)
        DB::table('provinces')->insertOrIgnore([
            ...
            ['name' => 'Forlì-Cesena',          'code' => 'FC', 'region' => 'Emilia-Romagna',        'is_active' => true],
    ...
    {
        // Non reversibile: i dati iniziali non vengono rimossi automaticamente
    }
};
